 ...
 * Tries both orders (password+salt and salt+password) to handle
 * different legacy implementations that may have used either convention.
 */
function verifyLegacySha256(password: string, storedPassword: string): boolean {
  const [salt, hash] = storedPassword.split(':');
  if (!salt || !hash) return false;

  // Try password+salt first (most common convention)
  const hash1 = createHash('sha256').update(password + salt).digest('hex');
  if (hash === hash1) return true;

  // Try salt+password (alternative convention used by some implementations)
  const hash2 = createHash('sha256').update(salt + password).digest('hex');
  return hash === hash2;
}
